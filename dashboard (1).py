...
#
# placeOrder() / placeOrderFullResponse() returns immediately.
# Order Book is used only once if the returned order ID is missing.
#
...
# ============================================================
# ORDER BOOK SNAPSHOT / ORDER-ID RECOVERY
# ============================================================

def get_order_book(api):
    """
    Read the current Angel One Order Book once.

    This is used ONLY to recover an order ID if the placement
    response/SDK does not expose it. It does NOT wait for
    execution and does NOT confirm a fill.
    """
    try:
...
def recover_new_order_id(
    api,
    option,
    quantity,
    before_order_ids=None,
):
    """
    Recover the NEW order ID immediately from Order Book.

    It compares the Order Book with the snapshot taken immediately
    before placement, so an older order for the same CE is not
    accidentally treated as the new order.

    No execution confirmation is performed.
    """

    before_order_ids = before_order_ids or set()

    orders = get_order_book(api)

    if not orders:
        return "", None

    symbol = str(option["symbol"]).upper()
    ...
    for order in orders:
        ...
        # A new ID is preferred.
        is_new_id = oid not in before_order_ids
    ...
    # New order ID wins. Within that group, preserve Order Book
    # order (normally newest first).
    candidates.sort(
        key=lambda x: x[0],
        reverse=True,
    )
...
def automatic_buy_ce(
    ...
    """
    Submit ONE real BUY order for the selected ATM CE.

    Order ID logic:
      1. Snapshot Order Book IDs.
      2. Submit the real Angel One order.
      3. Read orderid directly from the placement response.
      4. If missing/exception, read Order Book ONCE and recover
         only a NEW matching order ID.

    This function does NOT poll for execution and does NOT wait
    for broker execution confirmation.
    """
    ...
    # --------------------------------------------------------
    # Snapshot existing order IDs BEFORE placement.
    # --------------------------------------------------------
    before_orders = get_order_book(api)
    before_ids = order_id_set(before_orders)

    response = None
    placement_exception = None
    ...
    try:
        ...
        placement_exception = e

    # --------------------------------------------------------
    # 1) Normal case: extract order ID directly.
    # --------------------------------------------------------
    if response:
    ...
    # --------------------------------------------------------
    # 2) Recovery case:
    #    placement response was empty/no ID OR SDK raised an
    #    exception after the request may have reached Angel One.
    #
    #    Read Order Book ONCE. No execution confirmation.
    # --------------------------------------------------------
    recovered_id, recovered_order = (
        recover_new_order_id(
            api=api,
            option=option,
            quantity=option["quantity"],
            before_order_ids=before_ids,
        )
    )
    ...
    if recovered_id:
        recovery_response = {
            ...
            "recovered_from_order_book": True,
            "orderid": recovered_id,
    ...
    # --------------------------------------------------------
    # Nothing gave us an order ID.
    # --------------------------------------------------------
    if placement_exception is not None:
        raise RuntimeError(
            "Angel BUY request raised an exception and no new "
            "matching Order ID was found in Order Book: "
            f"{placement_exception}"
        )

    if not response:
        raise RuntimeError(
            "Angel BUY returned an empty response and no new "
            "matching Order ID was found in Order Book."
        )

    raise RuntimeError(
        "Angel BUY response did not contain an Order ID and "
        "no new matching Order ID was found in Order Book. "
...
def process_automatic_buy(
    ...
    try:
        ...
        # Some Angel One responses/order-book records also expose
        # uniqueorderid. Save it when available, but do not use it
        # as a replacement for the actual orderid.
        try:
            unique_id = None
            ...
            if isinstance(response, dict):
                ...
                if not unique_id:
                    unique_id = (
                        ...
                        or response.get("uniqueOrderId")
                    )

            if not unique_id:
                unique_id = response.get("order", {}).get(
                    "uniqueorderid"
                ) if isinstance(response, dict) and isinstance(
                    response.get("order"), dict
                ) else None

            if unique_id:
                state["last_unique_order_id"] = str(
                    unique_id
                )
