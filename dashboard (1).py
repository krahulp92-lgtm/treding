...
def place_buy_ce(
    ...
    if PAPER_TRADING:
        order_id = "PAPER-" + now_ist().strftime("%Y%m%d%H%M%S%f")
        order = {
            "time": timestamp,
            "order_id": order_id,
            "mode": "PAPER",
    ...
    # LIVE ORDER
    # --------------------------------------------------------
    # Use Angel One's full-response method first. The official
    # SmartAPI SDK returns the broker order id as
    # response["data"]["orderid"]. placeOrder() can instead
    # return None on an API failure and hide the useful response.
    # --------------------------------------------------------
    try:
        if not hasattr(api, "placeOrderFullResponse"):
            raise RuntimeError(
                "Installed SmartAPI SDK does not provide "
                "placeOrderFullResponse(). Please update "
                "smartapi-python."
            )
        ...
        if is_rate_limit_error(exc):
            set_rate_limit()
        raise RuntimeError(
            "BUY FAILED - Angel One API exception: " + str(exc)
        )
    ...
    if isinstance(response, dict) and response.get("status") is False:
        message = response.get("message") or "Unknown broker rejection"
        errorcode = response.get("errorcode") or response.get("error_type") or ""
        detail = f" | errorcode={errorcode}" if errorcode else ""
        raise RuntimeError(
            f"BUY FAILED - Angel One rejected order: {message}{detail}"
        )

    # --------------------------------------------------------
    # Extract real broker order id. NEVER invent one.
    # --------------------------------------------------------
    order_id = extract_order_id(response)

    # --------------------------------------------------------
    # If the full response is empty/malformed, check the broker
    # order book once before declaring the result unknown.
    # This can recover the real order ID if Angel accepted the
    # order but the placement response was lost.
    # --------------------------------------------------------
    if not order_id:
        try:
            broker_orders = fetch_broker_order_book(api)
            recovered = find_recent_matching_order(
                broker_orders,
                symbol=option["symbol"],
                quantity=quantity,
                side="BUY",
            ...
            if recovered:
                order_id = str(
                    recovered.get("order_id") or ""
                ).strip() or None
        except Exception:
            recovered = None

    if not order_id:
        # DO NOT retry automatically. An empty placement response
        # is an UNKNOWN result, not proof that no order was accepted.
        order = {
            "time": timestamp,
            "order_id": "",
            "mode": "LIVE",
            "status": "UNKNOWN - VERIFY ORDER BOOK",
            "side": "BUY",
            "symbol": option["symbol"],
            "token": option["token"],
            "expiry": option["expiry_raw"],
        ...
        add_local_order(order)
        raise RuntimeError(
            "BUY UNKNOWN - Angel One returned no order ID and "
            "the Order Book did not contain a matching order. "
            "Automatic retry is blocked to prevent duplicate BUYs. "
